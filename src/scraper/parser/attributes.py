...
def parser_authors(bylines):
    ...
    if bylines:
        if isinstance(bylines, str):
            ...
            for author in bylines[0]['creators'] if bylines[0].get('creators') else bylines[0]['creatorSnapshots']:
                authors.append({
                    'name': author['displayName'],
                    'photos': parser_images(author['promotionalMedia']['crops']) if author.get('promotionalMedia') else None
                })
        else:
...
def parser_section(section):
    return {"name": section['name'], "displayName": section.get('displayName'), "url": urljoin('https://www.nytimes.com/', section['url']) if section.get('url') else None, 'uri': section.get('uri', get_uri(section['id']))} if section else None

def parser_social(social):
    return [{
        'type': contact['type'],
        'account': contact['account']}
    for contact in social
    ]
    ...
    for c,edge in enumerate(entries):
        ...
        if edge:
            new_entries.append({
                ...
                'summary': edge['summary'] if edge.get('summary') else edge.get('creativeWorkSummary'),
