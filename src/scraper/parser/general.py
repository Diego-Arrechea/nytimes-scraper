from nytimes_scraper.src.scraper.parser import load_data
from nytimes_scraper.src.scraper.parser import attributes
from nytimes_scraper.src.helpers import get_content_soup
...
def parser_search(response):
    ...
    for asset_type in asset_types_:
        types.append({
            "totalEntries": asset_type["assetCount"],
            "name": asset_type["assetType"]
        })

    return {"entries": entries, "sections": sections, "types": types, "totalEntries": totalEntries, 'pageInfo': pageInfo}

def parser_person(response):
    data = response.json()['data']['anyWork']
    if data == None: return None # person not found

    Name = data['displayName']
    Biography = load_data.get_soup(data['legacyData']['htmlBiography']).text
    Photo = attributes.parser_images(data['promotionalMedia']['crops']) if data['promotionalMedia'] else None
...
def parser_video(response):
    ...
    authors = attributes.parser_authors(files[data['bylines'][0]['id']]['renderedRepresentation']) if data['bylines'] else None
    language = files[data['language']['id']]['code'] if data['language'] else None
    published = data['firstPublished']
    modified = data['lastModified']
    section = attributes.parser_section(files[data['section']['id']]) if data['section'] else None
    subsection = attributes.parser_section(files[data['subsection']['id']]) if data['subsection'] else None
...
def parser_slideshow(response):
    ...
    title = attributes.parser_title(data)
    summary = data['summary']
    authors = attributes.parser_authors(get_content_soup(soup.find('meta', {'name': 'byl', 'data-rh': 'true'}), 'content'))
...
def parser_paidpost(response):
    ...
    language = get_content_soup(soup.find('meta', {'http-equiv': 'Content-Language', 'data-rh': 'true'}), 'content')
    published = data['firstPublished']
    modified = data['lastModified']
    section = attributes.parser_section(data['section'])
    subsection = attributes.parser_section(data['subsection'])
...
def parser_recipes(response):
    ...
    authors = [{"name": author['name'], 'url': 'https://cooking.nytimes.com' + author['link']} for author in data['contentAttribution']['primaryByline']['authors']]
    adapted = [{"name": author['name'], 'url': 'https://cooking.nytimes.com' + author['link']} for author in data['contentAttribution']['secondaryByline']['authors']]
    image = {"link": data['image']['src']['card'], "credit": data['image']['credit']}
    time = data['time']
    rating = data['ratings']['avgRating']
    ratingCount = data['ratings']['numRatings']
    relatedArticles = [{'name': article['headline'], 'author': article['byline'], 'url': article['url']} for article in data['relatedArticles']]
    relatedGuides = [{'author': guide['author'], 'image': guide['image']['src']['card'], 'title': guide['title'], 'url': 'https://cooking.nytimes.com' + guide['url']} for guide in data['relatedGuides']]
    nutritionalInformation = [{'text': nutritional['header'], 'information': nutritional['description'].split('; '), 'source': nutritional['source']} for nutritional in data['nutritionalInformation']]
    yield_ = data['recipeYield']
    ingredients = data['ingredients']
    steps = data['steps']
    tips = data['tips']
    tags = [{'name': tag['name'], 'url': 'https://cooking.nytimes.com' + tag['path']} for tag in data['tags']]
...
def parser_audio(response):
    ...
    subsection = attributes.parser_section(data['subsection']) if data['subsection'] else None
    language = get_content_soup(soup.find('meta', {'http-equiv': 'Content-Language', 'data-rh': 'true'}), 'content')
...
def parser_interactive(response):
    ...
    title = attributes.parser_title(data)
    summary = data['summary']
    section = attributes.parser_section(data['section']) if data.get('section') else None
    subsection = attributes.parser_section(data['subsection']) if data.get('subsection') else None
    authors = attributes.parser_authors(get_content_soup(soup.find('meta', {'name': 'byl', 'data-rh': 'true'}), 'content'))
    published = get_content_soup(soup.find('meta', {'property': 'article:published_time', 'data-rh': 'true'}), 'content')
    modified = get_content_soup(soup.find('meta', {'property': 'article:modified_time', 'data-rh': 'true'}), 'content')
    language = get_content_soup(soup.find('meta', {'http-equiv': 'Content-Language', 'data-rh': 'true'}), 'content')
...
def parser_article(response):
    ...
    summary = data['summary']
    language = data['language']['code'] if data['language'] else None
    authors = attributes.parser_authors(data['bylines']) if data.get('bylines') else None
    published = data['firstPublished']
    modified = data['lastModified']
    section = attributes.parser_section(data['section'])
    subsection = attributes.parser_section(data['subsection'])
    alteration = [{"text": endum['body']['text'], 'date': endum['publicationDate']} for endum in data['addendums']] if data.get('addendums') else None
