from nytimes_scraper.src.scraper.parser.general import *
from nytimes_scraper.src.exceptions import *

def detect_article(url):
	key = url.split('nytimes.com/')[1].split('/')[0]

	if key.isdigit() and len(key) == 4 and int(key) > 1800 or key == 'article' or key == 'live':
		return True
...
def check(url, typename):
    ...
    if type_ != typename:
        if type_ == False:
            ...
            message = 'You are using the get_{}() function and the url is a {}. You must use get_{}()'.format(typename, type_, type_)
